...
Interactive::Interactive(Vob* parent, World &world, const zenkit::VMovableObject& vob, Flags flags)
  ...
  setVisual(vob);
  mdlVisual = vob.visual->name;

  if(isLadder() && !mdlVisual.empty()) {
    // NOTE: there must be else way to determinate steps count, but for now - we parse filename
...
bool Interactive::setPos(Npc &npc,const Tempest::Vec3& pos) {
  ...
  auto p    = pos;
  if(!isLadder()) {
    auto ray = world.physic()->ray(p+Tempest::Vec3(0,100,0), p+Tempest::Vec3(0,-500,0));
    if(ray.hasCol)
      p.y = ray.v.y;
    }
  npc.setPosition(p);
...
Tempest::Vec3 Interactive::nodePosition(const Npc* npc, const Interactive::Pos &to) const {
  ...
  if(!isLadder()) {
    auto ray = world.physic()->ray(Tempest::Vec3(x,y+100,z), Tempest::Vec3(x,y-500,z));
    if(ray.hasCol)
      y = ray.v.y;
    }
